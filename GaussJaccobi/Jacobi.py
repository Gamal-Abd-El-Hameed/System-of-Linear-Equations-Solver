...
class Jacobi:
    ...
    def getAbsoluteRelativeError(self, newValue, oldValue): # method to calculate the relative error
        if (newValue == 0):
            return 100   # Assumption  instead of relative error = infinity
        ans = self.converter.convert(abs(self.converter.convert(self.converter.convert(newValue - oldValue) / newValue)))    # calculate the relative error
    ...
    def solve(self):
        ...
        for iteration in range(self.iterations):
            maxRelativeError = 0
            if finished:     # check whether we reach relativeError < eps
                break
            ...
            for i in range(len(self.newX)):
                ...
                for j in range(len(self.newX)):
                    ...
                    tempSum = self.converter.convert( tempSum + self.converter.convert( self.A[i][j] * oldX[j] ) )
